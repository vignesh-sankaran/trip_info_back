...
pub fn create_new_user<'a>(conn: &PgConnection, uuid: &'a str) -> UserInfo {
    ...
    let new_user = NewUser { uuid: uuid };
...
pub fn update_user_home<'a>(conn: &PgConnection,
                            a_uuid: &'a str,
                            a_home_address_text: &'a str,
                            a_home_address_lat: &'a str,
                            a_home_address_long: &'a str)
                            -> UserInfo {
    use self::schema::user_info::dsl::{user_info, home_address_text, home_address_lat,
                                       home_address_long};
...
pub fn update_user_destination<'a>(conn: &PgConnection,
                                   a_uuid: &'a str,
                                   a_destination_address_text: &'a str,
                                   a_destination_address_lat: &'a str,
                                   a_destination_address_long: &'a str)
                                   -> UserInfo {
    use self::schema::user_info::dsl::{user_info, destination_address_text, destination_address_lat,
                                       destination_address_long};
...
mod test {
    ...
    fn test_update_user_home() {
        ...
        let _ = super::update_user_home(&db_conn,
                                        uuid_string,
                                        &home_address_text_string,
                                        &home_address_lat_string,
                                        &home_address_long_string);
    ...
    fn test_update_user_destination() {
        ...
        let _ = super::update_user_destination(&db_conn,
                                               uuid_string,
                                               &destination_address_text_string,
                                               &destination_address_lat_string,
                                               &destination_address_long_string);
